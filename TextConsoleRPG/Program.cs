...
");
 ...
 ╚═════════════════════════════╝
   ...
   1. ⚔️ 전사   - 강인한 체력과 높은 방어력, 강한 근접 전투능력을 가진다.
   2. 🔮 마법사 - 다양한 속성의 마법으로 광역공격에 특화되어 있으며, 치유와 보조로 아군을 지원한다.
   3. 🏹 궁수   - 빠른 사격과 높은 치명타 확률로 적을 단숨에 제압한다.
   4. 🗡  도적   - 빠른 속도와 높은 회피 능력으로 적의 공격을 피하고, 기습 공격으로 치명상을 입힌다.
");
                        ...
                        Monster targetMonster = mm.spawnedMonsters[monsterIdx];
                        {
                            ...
                            else
                            {
                                ...
                                else //명중
                                {
                                    ...
                                    int total = partyMem.Damage(Atkf, 0, 0.15f);
                                    targetMonster.DamageByPlayer(total);
                                    Console.WriteLine("Enter 를 눌러주세요.");
                                    Console.ReadLine();
                                }
                        ...
                        if (partyMem.LearnedSkills[skillChoice].Type == 1)
                        {
                            ...
                            target.Add(mm.spawnedMonsters[MonsterIdx]);
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        player.UpdateQuest(target[0].Name, QUEST_TYPE.KILL_MONSTER);
